...
async function seed() {
  ...
  await db.query(
    `INSERT INTO users (id, email, password_hash, role, core_member, name, onboarding_completed)
     VALUES ($1, $2, $3, 'teacher', true, $4, true)`,
    [dummy.teacherId, dummy.teacherEmail, hash, `Browser Teacher ${RUN_ID}`]
  );
  await db.query(
    `INSERT INTO users (id, email, password_hash, role, name, onboarding_completed)
...
async function apiLogin(email) {
  const res = await fetch(`${API_ORIGIN}/v1/auth/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-device-id': `browser-e2e-${RUN_ID}` },
...
async function loginViaContext(context, email) {
...
async function main() {
  ...
  await loginViaContext(teacherCtx, dummy.teacherEmail);
  await loginViaContext(studentCtx, dummy.studentEmail);

  const teacherPage = await teacherCtx.newPage();
  const studentPage = await studentCtx.newPage();
  await injectPcHook(teacherPage);

  let teacherPageRef = teacherPage;
  try {
    report.metrics.teacherLoginMs = Date.now() - tLoginStart;

    log('teacher', `navigate ${teacherLiveUrl}`);
    await teacherPage.goto(teacherLiveUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });

    await teacherPage.locator('#live_name').waitFor({ state: 'visible', timeout: 45000 });
    const liveTitle = `Browser Live Test ${RUN_ID}`;
    const nameInput = teacherPage.locator('#live_name');
    await nameInput.click();
    await nameInput.fill(liveTitle);
    await teacherPage.waitForFunction(
      (expected) => (document.querySelector('#live_name')?.value || '') === expected,
      liveTitle,
      { timeout: 10000 }
    );
    const goLiveBtn = teacherPage.getByRole('button', { name: /go live/i });
    await goLiveBtn.waitFor({ state: 'visible', timeout: 15000 });
    await teacherPage.waitForFunction(
      () => {
        const btns = Array.from(document.querySelectorAll('button'));
        return btns.some((b) => /go live/i.test(b.textContent || '') && !b.disabled);
      },
      { timeout: 30000 }
    );
    ...
    await teacherPage.waitForURL(/\/live\/[0-9a-f-]{36}/i, { timeout: 60000 }).catch(async () => {
      const errText = await teacherPage.locator('text=/error|required|failed/i').first().textContent().catch(() => '');
      throw new Error(`Go Live navigation failed: ${errText || 'unknown'}`);
    });
    ...
    await teacherPage.getByText('ONGOING').waitFor({ timeout: 90000 });
    await teacherPage.getByText('R2 Live (HLS)').waitFor({ timeout: 30000 }).catch(() => {});
    ...
    // Stream long enough for LL-HLS segments → worker → R2
    log('live', 'streaming 35s for HLS segments → R2');
    await teacherPage.waitForTimeout(35000);
    ...
    await studentPage.goto(studentLiveUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    report.ok = false;
    try {
      const shot = path.join(__dirname, `r2-live-browser-error-${RUN_ID}.png`);
      if (typeof teacherPageRef !== 'undefined') {
        await teacherPageRef.screenshot({ path: shot, fullPage: true }).catch(() => {});
